...
const FoodDetails: React.FC = () => {
  ...
  const routeParams = route.params as Params; // obtendo o id de food
  ...
  useEffect(() => {
    async function loadFavoriteFood(): Promise<void> {
      ...
      const foodFound: Food[] = response.data.filter(
        (foodFiltered: Food) => foodFiltered.id !== food.id,
      );

      if (food.id === foodFound[0].id) {
        console.log('igual');
      } else {
        console.log('n achou');
      }
  ...
  function handleIncrementFood(): void {
    const totalFoodQuantity = foodQuantity + 1;

    setFoodQuantity(totalFoodQuantity);
  }
